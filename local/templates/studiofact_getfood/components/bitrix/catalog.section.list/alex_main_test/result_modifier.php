	...
	foreach ($arResult['SECTIONS'] as $key => $arSection)
		{
			...
			if($uf_value = $db_list->GetNext()){
										    // pr($uf_value [UF_CUISINE]);
				
				if($uf_value ['UF_CUISINE']){
	
					$res = CIBlockElement::GetByID($uf_value['UF_CUISINE']);
					if($arResult['SECTIONS'][$key]['CUISINE'] = $res->GetNext()){
					    $dbProperty = CIBlockElement::getProperty(
					    	$arResult['SECTIONS'][$key]['CUISINE']['IBLOCK_ID'], 
					    	$arResult['SECTIONS'][$key]['CUISINE']['ID'], array("sort", "asc"),
					    	array()
					    );
					    while ($arProperty = $dbProperty->GetNext()) {
					            $arResult['SECTIONS'][$key]['CUISINE'][$arProperty['CODE']] = $arProperty['VALUE'];
					    }
					}
				}
