 ...
 */
?>
<?//=$arResult['ITEM']['DAY_WEEK']?>  

<?
// pr($arResult['ITEM']['CODE']);
// pr($arResult['DAY_WEEK']);
?>
<div class="img_box">
		<p class="product-item-image-wrapper" title="<?=$imgTitle?>" data-entity="image-wrapper">
		...
		<span style="/*background-color: #000; opacity:0.5;*/">
			<span class="product-item-image-original" id="<?=$itemIds['PICT']?>" style="background-image: url(<?=$item['PREVIEW_PICTURE']['SRC']?>); display: <?=($showSlider ? 'none' : '')?>;"></span>
		</span>
	...
	<div class="<?if($arResult['DAY_WEEK']):?>hover_over<?else:?>product-blocked<?endif?>">
		<a href="javascript:;" data-width="810" data-fancybox="group" data-src="<?=$item["DETAIL_PAGE_URL"];?>?open_popup=Y&day_week=<?=$arParams["DAY_WEEK"];?>" class="open_fancybox" rel="gallery">
...
<div class="item-info">
	...
	<div class="bx_catalog_item_title name">
		<a href="<?=substr($url,0 , strrpos($url,'/')+1)?>" title="<?=$productTitle?>"><?=$productTitle?></a>
	</div>
	...
	<div class="control-container">
		<div class="bx_catalog_item_price" data-entity="price-block">
			<div class="item_price">
				...
				?>
				<div class="price_box__actual-price" id="<?=$itemIds['PRICE']?>" data-price="<?=$price['PRICE']?>">
					...
					if (!empty($price))
					{
						...
						else
						{
							...
							echo $price['PRICE']."&nbsp;<span class='rub black'>P</span>";

							// echo str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", CurrencyFormat($price['PRICE'], "RUB"));
						}
					}
					?>
			...
			<?
					if (empty($arResult['DAY_WEEK']))
					{
						?>
						<div class="product-not-available" rel="nofollow">сегодня товар не доступен</div>
						<?
					}
					?>
		</div>
		...
		<div class="bx_catalog_item_controls align-items-center">
			<div class="bx_catalog_item_controls_blockone item_quantity inline" data-entity="quantity-block">
				...
				if (!$haveOffers)
				{
					if ($actualItem['CAN_BUY'] && $arParams['USE_PRODUCT_QUANTITY'] && $arResult['DAY_WEEK'])
					{
						?>
			...
			<div class="buy_box fr" data-entity="buttons-block">
				<?
				// pr($arParams['BUY_BUTTON']);
				// pr($arParams['USER_TYPE']);
				$style = '';
				if(empty($arParams['BUY_BUTTON'])){
					if($arParams['USER_TYPE'] == 'user'){
						$style = 'style="pointer-events: none; cursor: default;background-color: #b7a8957d;"';
					}
				}

				if (!$haveOffers)
				{
					if ($actualItem['CAN_BUY'] && $arResult['DAY_WEEK'] && $arParams['HIDE_BASKET'] !== 'Y')
					{
						?>
						
						<div id="<?=$itemIds['BASKET_ACTIONS']?>" class="basket_action_container">
							<a class="buy buy_button_a show_basket_popup inline" id="<?=$itemIds['BUY_LINK']?>" href="javascript:void(0)" rel="nofollow" data-screen="desktop" <?=$style?>></a>
						</div>
						<?
					}
					...
					if ($actualItem['CAN_BUY'] && $arResult['DAY_WEEK'] && $arParams['HIDE_BASKET'] !== 'Y')
					{
						?>
						<div id="<?=$itemIds['BASKET_ACTIONS']?>" class="basket_action_container">
						<a href="javascript:;" data-width="810" data-fancybox="group" data-src="<?=$item["DETAIL_PAGE_URL"];?>?open_popup=Y&day_week=<?=$arParams["DAY_WEEK"];?>" class="buy buy_button_a open_fancybox" rel="gallery"></a>
						</div>
						<?
					}
	...
	<div class="product-params<?if((isset($actualItem['DISPLAY_PROPERTIES']) && !empty($actualItem['DISPLAY_PROPERTIES'])) || count($item['OFFERS']) > 0):?> top-indent<?endif;?>">
		...
		if (!$haveOffers)
		{
			...
			{
				?>
				<div class="bx_catalog_item_articul item_props main_preview_props" data-entity="props-block" style="display: none;">
					<?
			...
			if ($arParams['ADD_PROPERTIES_TO_BASKET'] === 'Y' && !empty($item['PRODUCT_PROPERTIES']))
			{
				...
				<div id="<?=$itemIds['BASKET_PROP_DIV']?>" style="display: none;">
					...
					<input type="hidden" name="day_week" value="Mo" >
