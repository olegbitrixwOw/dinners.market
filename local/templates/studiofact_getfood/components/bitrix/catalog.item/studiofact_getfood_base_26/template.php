...
if (isset($arResult['ITEM']))
{
	...
	<div class="item_element good_box bx_catalog_item <?if(empty($arResult['DAY_WEEK'])):?>mobile-hide<?endif?>" data-entity="items-row">
		...
		if (!$haveOffers)
		{
			$jsParams = array(
				...
				'VISUAL' => array(
					...
					'AJAX_FILE'=>'/local/ajax/basket.php',
		...
		?>
		<?if (isset($arResult['IS_CAROUSEL']) && $arResult['IS_CAROUSEL'] == 'Y'):
			$GLOBALS['jsCarouselCode'] .= 'var ' . $obName . " = new JCCatalogItem(" . CUtil::PhpToJSObject($jsParams, false, true) . ");";
		else:?>
			<script>
