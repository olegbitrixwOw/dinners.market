...
if ($_REQUEST['open_popup'] == 'Y') {
	// $template = 'popup17';
	$template = 'studiofact_getfood';
}
// $template = '';
// echo $template;
// var_dump($arParams['DAY_WEEK']); 
// var_dump($arParams['VISUAL']); 
...
$elementId = $APPLICATION->IncludeComponent(
	...
	array(
		...
		// параметры компонента catalog.store.amount
		'STORE_PATH' => $arParams['STORE_PATH'],
		'STORE_MAIN_TITLE' => $arParams['MAIN_TITLE'],
		'STORE_USE_MIN_AMOUNT' =>  $arParams['USE_MIN_AMOUNT'],
		'STORE_MIN_AMOUNT' => $arParams['MIN_AMOUNT'],
		'STORES' => $arParams['STORES'],
		'STORE_SHOW_EMPTY_STORE' => $arParams['SHOW_EMPTY_STORE'],
		'STORE_SHOW_GENERAL_STORE_INFORMATION' => $arParams['SHOW_GENERAL_STORE_INFORMATION'],
		'STORE_USER_FIELDS' => $arParams['USER_FIELDS'],
...
if ($elementId > 0)
{
	if ($_REQUEST['open_popup'] != 'Y') // если это не быстрый просмотр
	{
		...
		if (!empty($recommendedData))
		{
			...
			$dbProp = CIBlockElement::GetProperty(
				...
				array(
					'CODE' => 'RECOMMEND' // ID товаров хранятся в свойстве "С этим товаром рекомендуем"
				)
			);

			$recommendedProductIds = [];
			while ($rsProp = $dbProp->Fetch())
			{
				$recommendedProductIds[] = $rsProp['VALUE'];
			}

			// очистка массива от пустых значений
			$recommendedProductIds = array_filter($recommendedProductIds, function($value) {
				return !empty($value);
			});

			if (CGetfood::getOption("RECOMENDS_TO_BUY") == 'true' && (!isset($arParams['DETAIL_SHOW_POPULAR']) || $arParams['DETAIL_SHOW_POPULAR'] != 'N') && !empty($recommendedProductIds))
			{
				?>
				<div class="catalog-block-header">
					<?=GetMessage('CATALOG_RECOMEND_TO_BUY')?>
				</div>

				<?
				global $arRecomendFilter;
				$arRecomendFilter = [
			 ...
			 * Big Data
			 */
			if (CGetfood::getOption("PERSONAL_RECOMEND") == 'true' && (!isset($arParams['USE_BIG_DATA']) || $arParams['USE_BIG_DATA'] != 'N')) {
				?>
				<div data-entity="parent-container">
					...
					$APPLICATION->IncludeComponent(
						...
						array(
							'IBLOCK_TYPE' => $arParams['IBLOCK_TYPE'],
							'IBLOCK_ID' => $arParams['IBLOCK_ID'],
							'SECTION_ID' => IBlock::getSectionIdByElementId($elementId, $arParams['IBLOCK_ID']),
							'SECTION_CODE' => $arResult['VARIABLES']['SECTION_CODE'],
			...
			if (
				...
				&& CGetfood::getOption("YOU_SEE") == 'true'
