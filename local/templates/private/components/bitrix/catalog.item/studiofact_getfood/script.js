(function (window){
	...
	window.JCCatalogItem.prototype = {
		...
		// 	// this.basketUrl = this.visual.AJAX_FILE + (this.basketMode === 'ADD' ? this.basketData.add_url : this.basketData.buy_url);
		// 	switch (this.productType)
		// 	{
		// 		case 1: // product
		// 		case 2: // set
		...
		initBasketUrl: function(){
			...
			let basketUrl = this.visual.AJAX_FILE + (this.basketMode === 'ADD' ? '/?action=ADD2BASKET&id=#ID#' : this.basketData.buy_url);
			...
			switch (this.productType)
			{
				...
				case 2: // set
						// this.basketUrl = this.basketUrl.replace('#ID#', this.product.id.toString());
						basketUrl = basketUrl.replace('#ID#', this.product.id.toString());
					break;
				case 3: // sku
						// this.basketUrl = this.basketUrl.replace('#ID#', this.offers[this.offerNum].ID);
						basketUrl = basketUrl.replace('#ID#', this.offers[this.offerNum].ID);
					break;
			}
		...
		{
			// !!!
			this.basketParams['day_week'] = this.visual.DAY_WEEK;
		...
		sendToBasket: function() 
		{
			...
			// if(this.getTeme() >= 19){
			// 	// console.log('заказ сделать нельзя');
			// 	this.initPopupWindow();

			// 	let MESSAGE = 'Вы можите делать заказа только до 10 часов утра текущего дня';
			// 	let popupContent = '<div style="width: 100%; margin: 0; text-align: center;"><p>'
			// 						+ (MESSAGE ? MESSAGE : BX.message('COMPARE_UNKNOWN_ERROR'))
			// 						+ '</p></div>';
			// 	let popupButtons = [
			// 						new BasketButton({
			// 							text: BX.message('BTN_MESSAGE_CLOSE'),
			...
			// 						})
			// 					];
			// 	this.obPopupWin.setTitleBar('Купить товар нельзя');
			// 	this.obPopupWin.setContent(popupContent);
			// 	this.obPopupWin.setButtons(popupButtons);
			// 	this.obPopupWin.show();
			...
			let basketUrl = this.initBasketUrl();
						...
						buttons = [
							...
							new BasketButton({
								...
								events: {
									/*!!!*/
									// click: BX.delegate(this.obPopupWin.close, this.obPopupWin) 
									click: BX.delegate(this.Reload, this)
								}
							})
						];
					}
