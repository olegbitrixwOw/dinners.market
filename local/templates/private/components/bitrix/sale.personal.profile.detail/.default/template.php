...
if(strlen($arResult["ID"])>0)
{
	...
	<form method="post"  class="col-md-12 sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
		...
		<?
		foreach($arResult["ORDER_PROPS"] as $k0 => $block)
		{
			if (!empty($block["PROPS"]))
			{
				...
				foreach($block["PROPS"] as $key => $property)
				{
					...
					<div class="form-group sale-personal-profile-detail-property-<?=strtolower($property["TYPE"])?>">
						<label class="sale-personal-profile-detail-form-label col-md-3 text-md-right <?=$alignTop?>" for="sppd-property-<?=$k0.$key?>">
							<?= $property["NAME"]?>:
							<?
								...
								else
								{
                                    ...
                                    <script>
                                        $(document).ready(function () {
                                            if($(".bx-ui-sls-fake").length){
                                                $(".bx-ui-sls-fake").attr("id", "sppd-property-<?=$k0.$key?>").addClass("form-control");
                                            }
                                        });
                                    </script>
                                <?
		...
		<div class="col-md-offset-3 col-sm-9 sale-personal-profile-btn-block">
			<input type="submit" class="btn-md" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
			&nbsp;
			<input type="submit" class="btn-md"  name="apply" value="<?=GetMessage("SALE_APPLY")?>">
			&nbsp;
			<input type="submit" class="btn-md"  name="reset" value="<?echo GetMessage("SALE_RESET")?>">
		</div>
	</form>
