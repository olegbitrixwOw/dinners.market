							...
							<div class="row">
								...
								<div class="col-md-4 col-sm-6 sale-order-detail-about-order-inner-container-price">
									<div class="sale-order-detail-about-order-inner-container-price-title">
										<?= Loc::getMessage('SPOD_ORDER_PRICE')?>: <?= $arResult["PRICE_FORMATED"]?>
									</div>

     									<?$deliveryPrice = deliveryPrice($arResult['ID']);?>
										<?if($deliveryPrice):?>
											<div class="sale-order-list-payment-price">  							
													<span class="sale-order-list-payment-number">Cтоимость доставки: <?=$deliveryPrice?> руб</span>
											</div>

											<div class="sale-order-list-payment-price">
												<span class="sale-order-detail-about-order-inner-container-status-detail">Общая стоимость заказа: <?=$deliveryPrice + $arResult["PRICE_FORMATED"]?> руб</span>
											</div>
										<?endif?>
								</div>
								<?if($arParams['USER'] && $arParams['USER']['TYPE'] == 'user'):?>
