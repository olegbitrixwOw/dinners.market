...
// if($arUser['TYPE'] !== 'admin' && $arUser['TYPE'] !== 'manager'){
if($arUser['TYPE'] == 'user'){
	return false;
}
...
if(strlen($arParams["USER_MESSAGE_ADD"]) <= 0){ 
	...
	if($arParams["IBLOCK_ID"] == IBLOCK_GILDS){
		$arParams["USER_MESSAGE_ADD"] = 'Организация добавлена';
	}elseif($arParams["IBLOCK_ID"] == IBLOCK_BALANCE){
		$arParams["USER_MESSAGE_ADD"] = 'Баланс пополнен';
	}
}
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			if($arParams["FIRM_ID"]){
				$sRedirectUrl = $sRedirectUrl.'&FIRM_ID='.$arParams["FIRM_ID"];
			}
