	...
	<table class="data-table" style="width: 90%">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
					<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]== "FIRM"){
					?>
					<input type="hidden" name="PROPERTY[<?=$propertyID?>][0]" value="<?=$arParams['FIRM_ID']?>"/>
					<?	continue;
					}?>
				<tr>
					<td>
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								<input <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]== "TIME"):?> type="time" <?else:?> type="text" <?endif?>  name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?=$value?>" /><br />
								<?
				...
				<td colspan="2">
					...
					<?if (strlen($arParams["LIST_URL"]) > 0):?>
						...
						<input
							...
							<? if($arParams['FIRM_ID']):?>
								onclick="location.href='<? echo CUtil::JSEscape($arParams["LIST_URL"]).'?FIRM_ID='.$arParams['FIRM_ID']?>';"
							<?else:?>	
