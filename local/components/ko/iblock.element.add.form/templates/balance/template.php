...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<?if ($arParams["FIRM_ID"] > 0):?><input type="hidden" name="FIRM_ID" value="<?=$arParams["FIRM_ID"]?>" /><?endif?>
	<table class="data-table" style="width: 90%">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				...
				<?
					// pr($propertyID);
					// pr(intval($propertyID) !== intval($arParams['FIRM_ID']));
					// echo intval($propertyID);
					// echo '<br>';
					// echo intval($arParams['FIRM_ID']);
				?>

				<?if (intval($propertyID) !== 80 && intval($propertyID) !== 84 && intval($propertyID) !== 85):?>
				<tr>
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								// if($propertyID!== $arParams['FIRM_ID']){
				...
				<?elseif($propertyID == 80):?>
						...
						for ($i = 0; $i<$inputNum; $i++){
							...
							?>
							<input type="hidden" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" value="<?=$arParams['FIRM_ID']?>" />
							<?
						}?>
				<?elseif($propertyID == 84):?>
						<?
						for ($i = 0; $i<$inputNum; $i++){
							if ($arParams["ID"] > 0 || count($arResult["ERRORS"]) > 0){
								$value = intval($propertyID) > 0 ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE"] : $arResult["ELEMENT"][$propertyID];
