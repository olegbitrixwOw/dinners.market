...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<table class="data-table" style="width: 90%">
		...
		<tbody>
			...
			<?	
				// if($propertyID == 65){
				// 	pr($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE']);
				// }
			?>
				...
				<tr>
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								value="<?=$value?>" 
								<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'PHONE'):?> data-input="phone" <?endif;?>
								/>
								<br />
								<?
					...
					<label for="file_delete_<?=$propertyID?>_<?=$i?>"><?=GetMessage("IBLOCK_FORM_FILE_DELETE")?></label><br />
										<?
			...
			<?//if($REQUST["edit"] == "Y"):?>
<!-- 				<tr>
					<td>ID пользователя который будет модератором огранизации:</td>
					<td><input type="text" name="USER_ID" maxlength="50" value=""></td>
				</tr> -->
			<?//endif?>
