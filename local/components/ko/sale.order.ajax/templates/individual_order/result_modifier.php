...
if(!empty($products) && !empty($prop_codes)){
    $arSelect = Array("ID", "IBLOCK_ID", "NAME", "DATE_ACTIVE_FROM","PROPERTY_*");//IBLOCK_ID è ID îáÿçàòåëüíî äîëæíû áûòü óêàçàíû, ñì. îïèñàíèå arSelectFields âûøå
...
// foreach ($arResult["JS_DATA"]["PAY_SYSTEM"] as $key => $pay_system) {
//     if($pay_system["PAY_SYSTEM_ID"]==9){
//         $arResult["JS_DATA"]["PAY_SYSTEM"][$key]["NAME"] = "Внутренний счет";
//         $arResult["JS_DATA"]["PAY_SYSTEM"][$key]["PSA_NAME"] = "Внутренний счет";
//     }
// }
// foreach ($arResult["JS_DATA"]["PAY_SYSTEM"] as $key => $pay_system) {
//     // if($pay_system['PAY_SYSTEM_ID'] != 10){
//     //     // unset($arResult["JS_DATA"]["PAY_SYSTEM"][$key]);
//     // }
//     pr($arResult["JS_DATA"]["PAY_SYSTEM"][$key]);
// }
// pr($arResult["JS_DATA"]);
...
// пролучаем кухню
if($arParams['CUISINE']){
    $arResult['CUISINE_EL'] = getCuisineElement($cuisine['cuisine']);
}
