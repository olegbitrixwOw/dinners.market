...
else
{
	...
	<?// if($arParams['USER'] !== 'admin' || $arParams['USER'] !== 'employee'):?>
	<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<input type="hidden" id="custId" name="PERSON_TYPE" value="1">
				...
				<div id="bx-soa-orderSave">
					...
					<!-- <a href="<?//=SITE_DIR;?>catalog/" style="margin: 10px 0" class="btn btn-catalog">
						<?//=GetMessage('MESS_TO_CATALOG')?>
					</a> -->
...
<?if($arResult['CUISINE_EL']):?>
	<script type="text/javascript">
		BX.message({
		    DELIVERY_TARIFF: '<?=$arResult['CUISINE_EL']['DELIVERY_TARIFF']?>',
		    DELIVERY_MAX_DISTANCE: '<?=$arResult['CUISINE_EL']['DELIVERY_MAX_DISTANCE']?>', 
		    DELIVERY_COST: '<?=$arResult['CUISINE_EL']['DELIVERY_COST']?>',
		    ADDRESS: '<?=$arResult['CUISINE_EL']['ADDRESS']?>',	    
		});
	</script>
	<?require_once('yandex_map.php');?>
<?endif;?>
<script>
  	document.addEventListener('input', function(event) {
		// console.log(event.target);
		if(event.target.id == 'soa-property-24'){			
			...
			console.log(address_home);

			if(address_street.value != '' && address_home.value != ''){
				ymaps.ready(init);
			}
		}
    });
