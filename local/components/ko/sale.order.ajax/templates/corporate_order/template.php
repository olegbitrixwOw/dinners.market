...
else
{
	...
	// echo $arParams['BALANCE'];
	
	?>
	<?if($limit):?>
	<?if($arResult['ORDER_TOTAL_PRICE'] < $arParams['BALANCE']):?>
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<? if ($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d'): ?>
					...
					<!--	DELIVERY BLOCK	-->
					<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active" <?=($hideDelivery ? 'style="display:none"' : '')?> ыенду>
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
	...
	?>
	<?else:?>
		<p>Вы не можите сделать заказ, так как сумма вашего заказа 
		больше суммы на балансе вашей организации.</p>
		<p>Обратитесь к вашему менеджеру чтобы он пополнил счет!</p>
	<?endif;?>
	<?endif;?>
	<?
}
