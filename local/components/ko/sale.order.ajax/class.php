...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		// комментируем // !!!
		// когда выставлен параметр Проверять email на уникальность при регистрации
		// незарегистрированным пользователям нельзя купить без авторизации
		// if (
		// 	$arParams['ALLOW_AUTO_REGISTER'] === 'Y'
		// 	&& $arParams['IS_LANDING_SHOP'] === 'N'
		// 	&& (
		...
		$this->day_week = $arParams['DAY_WEEK']; // !!!
		$this->limit =  $arParams['LIMIT'];
		$this->user =  (empty($arParams['USER']))? 'user' : $arParams['USER'];
		$this->organization = (empty($arParams['ORGANIZATION']))? 0 : $arParams['ORGANIZATION'];
		$this->delivery_address = (empty($arParams['DELIVERY_ADDRESS']))? 0 : $arParams['DELIVERY_ADDRESS'];
		$this->cuisine = (empty($arParams['CUISINE']))? 0 : $arParams['CUISINE'];
		
		// pr($arParams['ORGANIZATION']);
		// pr($this->organization);
		return $arParams;
	}
	...
	protected function getBasketStorage()
	{
		if (!isset($this->basketStorage))
		{
			...
			$this->basketStorage = Sale\Basket\Storage::getInstance(Sale\Fuser::getId(), $this->day_week); 
		}
			...
			if ($property['USER_PROPS'] === 'Y')
			{
				...
				{					
					if($property['CODE'] == 'NAME'){
						$curVal = $this->getValueFromCUser($property, true);
					}else{
			...
			if (empty($curVal))
			{
				...
				if ($property['TYPE'] !== 'LOCATION' || !$willUseProfile) 
				{
					...
					if ($shouldUseProfile && $USER->IsAuthorized())
					{
						...
						elseif($property['CODE'] == 'SECOND_NAME'){
							$curVal = $this->getValueFromCUser($property, true);
						}
						else{
							if($property['CODE'] == 'NAME'){
	 ...
	 * @return	string
	 */
	protected function getValueFromCUser($property, $filter = null)
	{
		...
		elseif ($property['IS_PAYER'] === 'Y')
		{
			...
			{
				// !!!
				// выводим имя, фамилию, отчество в поле свойства заказа
	...
	private function loadBasket()
	{
		...
		return $basketClassName::loadItemsForFUser(Sale\Fuser::getId(), $this->day_week);
	}
	...
	protected function showEmptyBasket()
	{
		...
		else
		{
			...
			$day_week = htmlspecialchars($this->request->get('day_week'));
			if($day_week){
				LocalRedirect($this->arParams['PATH_TO_BASKET'].'?day_week='.$day_week);
			}else{
				LocalRedirect($this->arParams['PATH_TO_BASKET'].'?day_week='.$day_week); 
			}
			...
			else
			{
				if ($USER->IsAuthorized())
				{
					...
					else
					{
						$day_week = htmlspecialchars($this->request->get('day_week'));
						if($day_week){
							LocalRedirect($APPLICATION->GetCurPage().'?day_week='.$day_week);
						}else{
	...
	protected function registerAndLogIn($userProps)
	{
		...
		// setLog($userData['NEW_NAME']);
		// setLog($userData['NEW_LAST_NAME']);
		// setLog($userProps['PROFILE_NAME']);
		// setLog($userData);

		// $pieces = explode(" ", $userProps['PROFILE_NAME']);
		// setLog($pieces);
		...
		$fields = [
			...
			'NAME' => $userData['NEW_NAME'],
			'LAST_NAME' => $userData['NEW_LAST_NAME'],
			'SECOND_NAME' => $userData['NEW_SECOND_NAME'],
			'PASSWORD' => $userData['NEW_PASSWORD'],
			'CONFIRM_PASSWORD' => $userData['NEW_PASSWORD_CONFIRM'],
			'EMAIL' => $userData['NEW_EMAIL'],
			'GROUP_ID' => $userData['GROUP_ID'],
		...
		else
		{
			...
			if ($USER->IsAuthorized())
			{
				if ($this->arParams['SEND_NEW_USER_NOTIFY'] == 'Y')
				{
					...
					// CHECK // !!!
					// активируем пользователя после регистрации
					$fields	= array( 
						"ACTIVE" => 'Y', 
						"UF_USER_TYPE" => USER_TYPE_USER
					); 
					$user->Update($userId, $fields);

					// отправляем письмо
					// setLog('пароль:'.$userData['NEW_PASSWORD'].' логин:'.$userData['NEW_LOGIN'].' email:'.$userData['NEW_EMAIL']);
					BitrixMail::send(array(
					    "EVENT_NAME" => "INDIVIDUAL_USER_REGISTRATION",
					    "LID" => "s1",
					    "C_FIELDS" => array(
					        'EMAIL' => $userData['NEW_EMAIL'],
					        'PASSWORD'  => $userData['NEW_PASSWORD'],
	...
	protected function autoRegisterUser()
	{
		...
		)
		{
			...
			if ($existingUserId > 0)
			{
				...
				// !!!
				// если пользовательне авторизован указал email уже зарегистрированного пользователя
				$this->addError(Loc::getMessage("STOF_ERROR_AUTH").' пользователь с таким номером телефона ('.$userProps['PHONE'].') или  email ('.$userProps['EMAIL'].') уже существует ', self::AUTH_BLOCK);
	...
	protected function recalculatePayment(Order $order)
	{
		...
		if ($remainingSum > 0 || $order->getPrice() == 0)
		{
			...
			if($this->user == 'user'){
					unset($this->arPaySystemServiceAll[PAY_SYSTEM_COMPANY]);
			}else{
				foreach ($this->arPaySystemServiceAll as $key => $pay_system) {
					if(!empty($this->limit)){
						if($pay_system['PAY_SYSTEM_ID'] != PAY_SYSTEM_COMPANY){ // оставляем внутренний счет компании
							unset($this->arPaySystemServiceAll[$key]);
						}
					}else{
						if($pay_system['PAY_SYSTEM_ID'] != PAY_SYSTEM_SBER){ // сбербанк
							unset($this->arPaySystemServiceAll[$key]);
						}
					}
				}
	...
	protected function saveOrderAjaxAction()
	{
		...
		if ($this->checkSession)
		{
			...
			// здесь проверяем дату если пользователь корпоративный
			if($this->user !== 'user'){
				$factual_day = substr(date("l"), 0, 2); // фактический день недели
				if($factual_day == $this->day_week){
					$currentTime = date('H:i'); 
					$currentDateTime = strtotime(date('d.m.Y')  ." ". $currentTime); 
					$stopDateTime = strtotime(date('d.m.Y')  ." ".$this->delivery_address['TIME']);
					
					if($stopDateTime <= $currentDateTime){	
						// запускаем функцию
						$arOrderRes['ERROR'] = 'TIME';
			...
			if (empty($this->arResult["ERROR"]))
			{
				...
				// здесь вызываем ORMhelper и проверяем есть ли запись с ID местоположения
				// запис нет - записываем  код организации, местоположение, сумму 
				// запись есть - обновляем сумму и число заказов
				
				// setLog('ORMhelper'); 

				// setLog($this->order->getPrice()); 
				// setLog('organization: '.$this->organization['ID']; 
				// setLog('delivery: '.$this->delivery_address['ID']); 
				// setLog('day: '.$this->day_week); 

				// if($this->organization && $this->day_week == substr(date("l"), 0, 2)){
				if($this->organization && $this->day_week == 'Fr'){
					$elFields = array(
						'DELIVERY_ADDRESS_ID' => $this->delivery_address['ID'],
						'SUM' => $this->order->getPrice(),
						'DAY' => $this->day_week,
						'TIME' => $this->delivery_address['TIME']
					);
					if(DayorderHelper::GetElement($this->delivery_address['ID'])){
						$elementUp = DayorderHelper::UpdateElement($elFields);
					}
					else{						
						$elFields['ORGANIZATION_ID'] = $this->organization['ID'];
						$elFields['TITLE'] = $this->organization['NAME'];
						$elFields['MIN_SUM'] = $this->delivery_address['MIN_SUM'];
						$elFields['DELIVERY_COST'] = $this->delivery_address['DELIVERY_COST'];
						$element = DayorderHelper::AddElement($elFields);
						// setLog($element);
					}
				}
			}
	...
	protected function getJsDataResult()
	{
		...
		// pr($arr);
		// pr($this->arParams['USER']);
		// !!!

		foreach ($arr['properties'] as $key => $property)
		{
			if($this->arParams['USER'] !== 'employee'){
				if($property['CODE'] == 'ORGANIZATION' || $property['CODE'] == 'DELIVERY_ADDRESS'){
					unset($arr['properties'][$key]);
				}
			}
			else{
				// if($property['CODE'] == 'ADDRESS_STREET' || $property['CODE'] == 'ADDRESS_HOME' || $property['CODE'] == 'ADDRESS_FLAT'){
				// 	unset($arr['properties'][$key]);
				// }
			}
	...
	protected function saveOrder($saveToSession = false)
	{
		...
		if ($res->isSuccess())
		{
		    ...
		    $somePropValue->setValue($this->day_week);

		    if($this->organization){
		    	$somePropValue = $propertyCollection->getItemByOrderPropertyId(ORGANIZATION);
		    	$somePropValue->setValue($this->organization['ID']);
		    }

		    if($this->delivery_address){
		    	$somePropValue = $propertyCollection->getItemByOrderPropertyId(DELIVERY_ADDRESS);
		    	$somePropValue->setValue($this->delivery_address['ID']);
		    }

		     if($this->cuisine){
		    	$somePropValue = $propertyCollection->getItemByOrderPropertyId(CUISINE);
		    	$somePropValue->setValue($this->cuisine);
		    }

		    // сохраняем пустые строки в обязательные свойства
		 	// 	if($this->user == 'employee'){
				// 	$ADDRESS_STREET = $propertyCollection->getItemByOrderPropertyId(23);
				// 	$ADDRESS_STREET->setValue('');

				// $ADDRESS_HOME = $propertyCollection->getItemByOrderPropertyId(24);
				// $ADDRESS_HOME->setValue('');

				// 	$ADDRESS_FLAT = $propertyCollection->getItemByOrderPropertyId(25);
				// 	$ADDRESS_FLAT->setValue('');
			// }
