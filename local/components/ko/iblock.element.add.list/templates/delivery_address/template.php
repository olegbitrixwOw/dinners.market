...
<?if($arResult["NO_USER"] == "N"):?>
	<?if($arUser['TYPE'] !== 'admin'):?>
		<thead>
			<tr>
				<td><?=GetMessage("IBLOCK_DELIVERY_ADDRESS")?></td>
				<td>Минимальная сумма заказа на день</td>
				<td>Стоимость доставки</td>
				<td>Время</td>
				<td>Общая стоимость (доставка + заказы)</td>
			</tr>
		</thead>
		<tbody>
		<?if (count($arResult["ELEMENTS"]) > 0):?>
			...
			<tr>
				...
				<td><?=$arElement["MIN_SUM"]["VALUE"]?></td>
				<td><?=$arElement["DELIVERY_COST"]["VALUE"]?></td>
				<td><?=$arElement["TIME"]["VALUE"]?></td>
				<td>
					<?$sum = DayorderHelper::showOrderSum($arElement["ID"])?>
					<?if($sum):?>
						доставка: <?if($sum['DELIVERY_COST']):?><?=$sum['DELIVERY_COST']?><?else:?>бесплатна<?endif?>
						общая сумма заказа с доставкой: <?=$sum['ORDER_SUM']?>
					<?endif?>						
				</td>
			</tr>
			<?endforeach?>
