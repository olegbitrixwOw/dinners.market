...
if (CModule::IncludeModule("iblock"))
{
	...
	if ($bAllowAccess)
	{
		...
		if ($USER->GetID())
		{
			...
			if($arParams["FIRM_ID"]){
				$arFilter["=PROPERTY_ORGANIZATION"] = $arParams["FIRM_ID"];
			}

			if($arParams["ID"]){
				// $rsIBlockElements = CIBLockElement::GetList(array(), array_merge($arFilter, array("ID" => $arParams["ID"])),  array("ID", "NAME", "DETAIL_TEXT", "PROPERTY_SUM"));
				$rsIBlockElements = CIBLockElement::GetList(array(), array_merge($arFilter, array("ID" => $arParams["ID"])));
			}else{
		...
		$arResult["ELEMENTS_COUNT"] = $rsIBlockElements->SelectedRowsCount();
			...
			while ($arElement = $rsIBlockElements->NavNext(false))
			{
				...
				$query = CIBlockElement::GetByID($arElement["ID"]);
				
				if($obRes = $query->GetNextElement())
				{
				  $arElement["SUM"] = $obRes->GetProperty("SUM");
				  $arElement["BALANCE"] = $obRes->GetProperty("BALANCE");
				  $arElement["FIRM"] = $obRes->GetProperty("ORGANIZATION");
				  $arElement["DELIVERY_COST"] = $obRes->GetProperty("DELIVERY_COST");
				  $arElement["MIN_SUM"] = $obRes->GetProperty("MIN_SUM");
				  $arElement["TIME"] = $obRes->GetProperty("TIME");
				  $arElement["INN"] = $obRes->GetProperty("INN");
				  $arElement["UR_ADDRESS"] = $obRes->GetProperty("UR_ADDRESS");
				}
