...
<? if (count($arResult['CATEGORIES']['READY']) > 0) { ?>
    ...
    <div class="small_basket_hover_block<? if ($_REQUEST["SMALL_BASKET_OPEN"] == "Y") { echo ' active'; } ?>">
         ...
         <? if ($arParams["ACTIVE_DAY_WEEK"] !== $arParams["DAY_WEEK"]): ?>
                <span class="notice">чтобы поменять параметры перейдите в этот день</span>
            <? endif;?>
            ...
            <table class="small_basket_hover_table">
                <?foreach ($arResult['CATEGORIES']['READY'] as $arItem):?>
                    ...
                    <td class="small_basket_hover_name">
                        ...
                        <div class="item_quantity small_basket_hover_quantity" data-cart-id="<?=$cartId?>" data-siteid="<?=SITE_ID?>" data-ajax-path="<?=$componentPath?>/ajax.php" data-template-name="<?=$templateName?>">
        ...
        <div class="small_basket_hover_block__buttons clearfix">
            <? if ($arResult["NUM_PRODUCTS"] > 0) { ?>
                <a href="#<?//=$arParams["PATH_TO_BASKET"];?>" class="small_basket_block">
            <? } else { ?>
                <span class="fr" class="small_basket_block"> 
            <? } ?>
                ...
                <span class="desktop"   <? if ($summ == 0):?> style="display: none;" <?endif ?>>
                        ...
                        <? if ($summ > 0) { ?>
                          <!--   <span class="summ inline"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", CurrencyFormat($summ, "RUB"));?></span> -->
                            <span class="summ inline">
                                <span class="desktop_price"><?=$summ;?></span> 
                                <span class="rub black"><?=GetMessage("STUDIOFACT_R")?></span>
                            </span>
