				...
				<tbody>
					...
					foreach ($arResult["ITEMS"]["AnDelCanBuy"] as $key => $arItem) {
						...
						<tr class="good_box">
							...
							<td class="itemAction">
								<? if ($bDeleteColumn): ?>
									<? if ($arParams["DAY_WEEK"]){
										$arUrls["delete"] = $arUrls["delete"].'&day_week='.$arParams["DAY_WEEK"];
										$arUrls["delay"] = $arUrls["delay"].'&day_week='.$arParams["DAY_WEEK"];
									}?>
					...
					<div class="basket_items_blocks_item">
						...
						<div class="itemAction">
							<? if ($bDeleteColumn): ?>
								<? if ($arParams["DAY_WEEK"]){
										$arUrls["delete"] = $arUrls["delete"].'&day_week='.$arParams["DAY_WEEK"];
										// arUrls["delete"] = '?day_week='.$arParams["DAY_WEEK"];
										$arUrls["delay"] = $arUrls["delay"].'&day_week='.$arParams["DAY_WEEK"];
									}?>
								<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"><?=GetMessage("SALE_DELETE")?></a>
							<? endif;
			...
			<div class="bx_ordercart_order_pay">
				...
				<div class="bx_ordercart_order_pay_center">
					...
					<?endif;?>
						...
						<a href="javascript:void(0);" class="btn_buy_one_click button" data-fancybox="" data-src="#allBuyModal">Купить корзины на все дни недели</a>

						<?if($arParams['DAY_WEEK']):?>
							<a href="/personal/order/make/?day_week=<?=$arParams['DAY_WEEK']?>" class="checkout button"><?=GetMessage("SALE_ORDER")?></a>
						<?else:?>
							<a href="/personal/order/make/" class="checkout button"><?=GetMessage("SALE_ORDER")?></a>
						<?endif;?>
				</div>
			</div>
		<? } else {
