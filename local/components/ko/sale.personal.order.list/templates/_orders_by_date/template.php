...
?>
	...
	?>
	    ...
	    <?if($arParams["NAVYPARAMS"]["TOTALPAGES"]):?>
		    <select class="fields boolean" name="page" tabindex="0" id="page">
		    	<?for($i=1; $i <= $arParams["NAVYPARAMS"]["TOTALPAGES"]; $i++):?>
		            <option value="<?=$i?>" <?if($arParams["NAVYPARAMS"]["PAGEN"] == $i):?>selected="selected"<?endif?>>страница <?=$i?></option>
		         <?endfor?>
		    </select>
	    <?endif?>
			...
			<div class="col-md-12 col-sm-12 sale-order-list-container">
				...
				<div class="row">
					<div class="col-md-12 sale-order-list-inner-container">
						...
						?>
						<?
						// echo $order["ORDER"]["URL_TO_DETAIL"];
						// pr($order["ORDER"]["URL_TO_DETAIL"]);
...
<script type="text/javascript">
    ...
    BX.ready(function(){
        ...
        $(document).on('change', '#page', function (e) {
            let page = $(this).val();
            let date = $('#datetimepicker').find("input").val();
            ajax(date, page);
        });
    });
