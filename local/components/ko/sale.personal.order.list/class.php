...
class CBitrixPersonalOrderListComponent extends CBitrixComponent
{
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		if(!empty($arParams["PATH_TO_DETAIL"])){
			$arParams["PATH_TO_DETAIL"] = $arParams["PATH_TO_DETAIL"].'?'."ID=#ID#";
		}else{
		...
		// $arParams["PATH_TO_DETAIL"] = $arParams["PATH_TO_DETAIL"].'?'."&USER_ID=".$arParams['USER_ID'];
	...
	protected function prepareFilter()
	{
		...
		// !!!
		$USER_ID = intval($this->arParams["USER_ID"]); 
		if($USER_ID){
			$arFilter["USER_ID"] = $USER_ID; 
		}
		// else{			
		// 	if($this->arParams["USER"]["TYPE"] !== 'manager' && $this->arParams["USER"]["TYPE"] !== 'admin'){
		...
		if($this->arParams["DATE"]){
			// pr($this->arParams["DATE"]);
			$date_insert_from = new \Bitrix\Main\Type\DateTime(date_create_from_format('d.m.Y', $this->arParams["DATE"])->format($this->arParams["DATE"].' 00:00:00'));
			$date_insert_to = new \Bitrix\Main\Type\DateTime(date_create_from_format('d.m.Y', $this->arParams["DATE"])->format($this->arParams["DATE"].' 23:59:59')); 
		...
		// !!!
		if($this->arParams["ORGANIZATION"]){
			$arFilter['PROPERTY.ORDER_PROPS_ID'] = ORGANIZATION;
            $arFilter['PROPERTY.VALUE'] = intval($this->arParams["ORGANIZATION"]);
		}
	...
	protected function obtainDataOrders()
	{
		...
		// !!!
		// $filter = array(
		// 	0=>array('!@STATUS_ID' => array(0 => 'F'),'CANCELED' => 'N'),
		// 	'PROPERTY.ORDER_PROPS_ID' => $this->filter['PROPERTY.ORDER_PROPS_ID'],
  		//  'PROPERTY.VALUE' => $this->filter['PROPERTY.VALUE'],
  		//  '>=DATE_INSERT' => $this->filter['>=DATE_INSERT'],
  		//  '<=DATE_INSERT' => $this->filter['<=DATE_INSERT'],
		// );
		...
		if($this->arParams['PAGE']){
			$navyParams['PAGEN'] = $this->arParams['PAGE'];
			$this->arParams['NAVYPARAMS'] = $navyParams;
		}
		...
		else
		{
			...
			if($this->arParams['PAGE']){
				$this->arParams['NAVYPARAMS']['TOTAL'] = $totalCount;
				$this->arParams['NAVYPARAMS']['TOTALPAGES'] = $totalPages;
			}
		}
		...
		if(self::isNonemptyArray($this->dbResult['ORDERS']))
		{
			foreach ($this->dbResult['ORDERS'] as $k => $orderInfo) 
			{
				...
				if($this->arParams["AJAX"] !=='Y'){
					...
					$arOrder["URL_TO_DETAIL"] = CComponentEngine::makePathFromTemplate(
						$this->arParams["PATH_TO_DETAIL"], array("ID" => urlencode(urlencode($arOrder["ACCOUNT_NUMBER"]))));	
					// pr($arOrder["URL_TO_DETAIL"]);
				}
